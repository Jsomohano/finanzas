...
import type { MonthComparisonRow } from '@/lib/db/types';
...
export function MonthComparison({ rows, labelA, labelB }: Props) {
  if (rows.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Comparativa mes vs. mes</CardTitle>
        </CardHeader>
        <CardContent>
        ...
        <div className="h-[220px]">
          <ResponsiveContainer>
            <BarChart data={chartData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              ...
              <Bar dataKey={labelB} fill="hsl(var(--muted-foreground))" radius={[3, 3, 0, 0]} opacity={0.5} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        {/* Delta badges */}
